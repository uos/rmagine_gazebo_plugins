#ifndef RMAGINE_GAZEBO_PLUGINS_GZ_RMAGINE_VULKAN_SENSOR_SYSTEM_HPP
#define RMAGINE_GAZEBO_PLUGINS_GZ_RMAGINE_VULKAN_SENSOR_SYSTEM_HPP
...
// + ros_gz_bridge covers it). Modeled on RmagineOptixSensorInstance's
// output-param simulate()/device-memory-download shape (Vulkan's
// SimulatorVulkan API matches OptiX's here, not Embree's simpler RAM-return
// one) but, like Embree, has no noise model support -- rmagine has no
// NoiseVulkan equivalent to NoiseCuda yet.
class RmagineVulkanSensorInstance
{
public:
  void Load(
...
private:
  ...
  rmagine::VulkanMapPtr map_;
  std::shared_ptr<std::shared_mutex> map_mutex_;
  SensorModelConfig model_cfg_;
  rmagine::SphereSimulatorVulkanPtr sim_spherical_;
  rmagine::PinholeSimulatorVulkanPtr sim_pinhole_;
  rmagine::O1DnSimulatorVulkanPtr sim_o1dn_;
  rmagine::OnDnSimulatorVulkanPtr sim_ondn_;
...
// discovers `<sensor type="custom" gz:type="rmagine_vulkan">`.
class RmagineVulkanSensorSystem
  : public gz::sim::System,
    public gz::sim::ISystemConfigure,
    public gz::sim::ISystemPostUpdate
{
public:
  RmagineVulkanSensorSystem() = default;
  ~RmagineVulkanSensorSystem() override = default;
...
private:
  ...
  std::unordered_map<gz::sim::Entity, std::unique_ptr<RmagineVulkanSensorInstance>> instances_;
};

}  // namespace rmagine_gazebo_plugins

#endif  // RMAGINE_GAZEBO_PLUGINS_GZ_RMAGINE_VULKAN_SENSOR_SYSTEM_HPP
